...
export class InvoiceService {
  ...
  protected convertDateFromClient(invoice: IInvoice): IInvoice {
    const copy: IInvoice = Object.assign({}, invoice, {
      orderplaced: invoice.orderplaced && invoice.orderplaced.isValid() ? invoice.orderplaced.format(DATE_FORMAT) : undefined
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.orderplaced = res.body.orderplaced ? moment(res.body.orderplaced) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((invoice: IInvoice) => {
        invoice.orderplaced = invoice.orderplaced ? moment(invoice.orderplaced) : undefined;
      });
    }
    return res;
  }
